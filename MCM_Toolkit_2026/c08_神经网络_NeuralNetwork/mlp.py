# -*- coding: utf-8 -*-
# MLP（多层感知机）：分类 / 回归（基于 scikit-learn）
...
def solve(data: Any, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        params: 参数：
            ...
            - model_params: dict（传给 MLPClassifier/MLPRegressor）
    ...
    if task == "classification":
        model = MLPClassifier(random_state=random_state, max_iter=500, **model_params)
    elif task == "regression":
        model = MLPRegressor(random_state=random_state, max_iter=1000, **model_params)
    else:
...
if __name__ == "__main__":
    ...
    y = 1.5 * X[:, 0] - 2.0 * X[:, 1] + rng.normal(scale=0.3, size=400)
    out = solve((X, y), params={"task": "regression"})
    print(out["metrics"])
