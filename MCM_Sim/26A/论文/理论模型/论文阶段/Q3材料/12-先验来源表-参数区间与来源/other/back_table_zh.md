...
| P0_screen | 屏幕点亮基准功耗（W） | logU[0.102, 0.190] | logU[0.132, 0.161] | 参量估计（AndroWatts 聚合表）：P0≈0.146W。用于屏幕亮度模型的截距项。 | 论文/理论模型/论文阶段/参量估计/01-亮度-屏幕功耗/fit_results.csv（来自 AndroWatts，CC BY 4.0） |
| k_screen | 亮度→功耗线性系数（W/nit） | logU[5.33e-04, 9.90e-04] | logU[6.86e-04, 8.38e-04] | 参量估计（AndroWatts 聚合表）：k≈7.62e-04 W/nit（Brightness->nits 映射见参量估计说明）。 | 论文/理论模型/论文阶段/参量估计/01-亮度-屏幕功耗/fit_results.csv（AndroWatts，CC BY 4.0） |
| k_cpu | CPU 负载功耗系数（P=k·load，W） | logU[1.73, 4.05] | logU[2.60, 3.18] | 参量估计（AndroWatts 聚合表）：k_cpu≈2.89W（通过 freq proxy 归一化负载回归）。 | 论文/理论模型/论文阶段/参量估计/02-负载-CPU_GPU功耗/fit_results.csv（AndroWatts，CC BY 4.0） |
| k_gpu | GPU 负载功耗系数（P=k·load，W） | logU[0.22, 0.52] | logU[0.33, 0.41] | 参量估计（AndroWatts 聚合表）：k_gpu≈0.372W（通过 GPU freq proxy 归一化负载回归）。 | 论文/理论模型/论文阶段/参量估计/02-负载-CPU_GPU功耗/fit_results.csv（AndroWatts，CC BY 4.0） |
| e_wifi | Wi-Fi 每 MB 传输能耗（J/MB） | logU[0.090, 0.241] | logU[0.135, 0.165] | 参量估计（AndroWatts）：e≈0.150 J/MB（能量守恒反推，含尾态能量项）。 | 论文/理论模型/论文阶段/参量估计/03-网络回落曲线/fit_results.csv（AndroWatts，CC BY 4.0） |
| tau_tail_wifi | Wi-Fi 尾态等效时间常数 τ_tail（s） | logU[13.3, 24.7] | logU[17.1, 20.9] | 参量估计（AndroWatts）：τ≈19.0s（定义：τ=E_tail/(P_conn-P_idle)，用于解释拖尾）。 | 论文/理论模型/论文阶段/参量估计/03-网络回落曲线/fit_results.csv（AndroWatts，CC BY 4.0） |
| R_th | 热阻 R_th（K/W） | logU[0.470, 0.874] | logU[0.605, 0.739] | 参量估计（AndroWatts 温升统计量反推）：R_th≈0.672 K/W（用于集总热模型量级）。 | 论文/理论模型/论文阶段/参量估计/05-温度-时间曲线/fit_results.csv（AndroWatts，CC BY 4.0） |
| C_th | 热容 C_th（J/K） | logU[13.18, 24.48] | logU[16.95, 20.71] | 参量估计（AndroWatts 温升统计量反推）：C_th≈18.83 J/K（由 τ=R_th*C_th 得到）。 | 论文/理论模型/论文阶段/参量估计/05-温度-时间曲线/fit_results.csv（AndroWatts，CC BY 4.0） |
