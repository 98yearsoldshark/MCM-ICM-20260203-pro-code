...
- 场景映射：采用规则把 open data 行粗分到 S1..S5，并在图中标注每类样本数 n；详见脚本源码。

复现命令：

```bash
PYTHONPATH=MCM_Sim/26A/src python MCM_Sim/26A/src/scripts/make_q1_tte_radar_with_opendata_support.py --mode both --lang en
```
